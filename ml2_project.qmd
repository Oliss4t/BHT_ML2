...
-   region: Four locations: northeast, southeast, southwest, northwest. All refer to the United States
...
Most of the individuals have no children, the mean is 1 child and the maximum is 5. Regarding the factor feature smoker the dataset is unbalanced. Only 20.5% of our individuals are smoker, respectively 79.5% are non smoker. The region is well balanced again between the four categories "southwest","southeast","northwest" and "northeast".
...
Next, we want to better understand the relationships between our different variables in the dataset. Therefore we use the correlation matrix to get insights about our data regarding the linearity and their respective strengths. A positive correlation means that as one variable increases, the other variable also increases, and a negative correlation means that as one variable increases, the other variable decreases. A correlation value of 1 or -1 indicates a perfect linear relationship between the variables. A correlation value of 0 indicates that there is no linear relationship between the variables. 
...
The R2 score of 0.71 is good enough for interpreting the coefficients. From the model summary we can see hat the following explanatory variables are statistically significant:
...
The best result was for the final tree found via hyperparameter optimisation which gave an RMSE of \[1\] 5076.103 and an R2 of \[1\] 0.8478985 on the validation data.


The tree once again looks as follows: 
```{r}
printcp(rpart(charges~.,train))
rpart.plot(rpart(charges~.,train))
```
...
On both of our chosen metrics, it is clear that the regression tree outperforms the baseline linear model. In turn, the Neural Network outperforms both of these. However, while there is a significant difference in terms of how the tree model performed over the linear model, the increase in performance of the neural network over the tree model is comparatively small. Because of this, it could be preferable to choose the tree model as or model of choice over the neural network, as it allows a greater explainability, in terms of how it can be visualised, easily explained to the non-expert, and understood, when compared to the neural network which contains some of the 'black box' type problems which some neural networks are prone to i.e. we cannot easily 'see inside' it to know what it is making its decisions based on.


*Talk about doing the Gridsearch for the splitting criteria as well, gini index vs. entropy
*Criticism of tree that it only predicts 4 values for what is a continuous variable
*Upside of linear model is that it is possibly the most easy to interpret of all three, even if it does not perform as well
